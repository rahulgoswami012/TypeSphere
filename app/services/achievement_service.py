...
from app.utils.timezone import get_ist_hour

BADGES_CATALOG = [
    ('first_test', 'First Contact', 'Complete your very first typing benchmark on TypeSphere.', 'zap'),
    ...
    ('perfect_acc', 'Flawless Flow', 'Complete a full benchmark with exactly 100% accuracy.', 'shield'),
    ('night_owl', 'Night Owl', 'Complete a verified typing run between 12:00 AM and 5:00 AM IST.', 'moon'),
    ('early_bird', 'Early Bird', 'Complete a verified typing run between 5:00 AM and 8:00 AM IST.', 'sun'),
    ('survival_master', 'Survival Master', 'Survive all 5 battery shield cells in Survival Mode.', 'heart'),
    ('certified_typist', 'Certified Scholar', 'Generate an official TypeSphere Flight Certificate.', 'file-text'),
...
class AchievementService:
    ...
    def check_and_award(user):
        ...
        existing_codes = {ua.achievement.code for ua in user_achievements if ua.achievement}
        ...
        # Night Owl & Early Bird (Evaluated against Indian Standard Time)
        if any(0 <= (t.time_of_day_ist if t.time_of_day_ist is not None else get_ist_hour(t.completed_at)) < 5 for t in tests) and 'night_owl' not in existing_codes:
            earned_codes.append('night_owl')
        if any(5 <= (t.time_of_day_ist if t.time_of_day_ist is not None else get_ist_hour(t.completed_at)) < 8 for t in tests) and 'early_bird' not in existing_codes:
